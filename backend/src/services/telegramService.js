...
⚠️ <b>Важно!</b> Сообщите этот Chat ID администратору при создании вашего аккаунта партнера, чтобы получать уведомления.
  ...
  🔗 <a href="${process.env.FRONTEND_URL || 'http://delta.freedomgroup.online/'}">Перейти в личный кабинет</a>
          ...
          [
            {
              text: '🌐 Открыть личный кабинет',
                url: process.env.FRONTEND_URL || 'http://delta.freedomgroup.online/'
            }
          ]
        ]
      }
...
🔗 <a href="${process.env.FRONTEND_URL || 'http://delta.freedomgroup.online/'}">Сайт партнерской системы</a>
    `.trim();
